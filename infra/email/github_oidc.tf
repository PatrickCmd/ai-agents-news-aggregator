...
resource "aws_iam_role" "gh_actions_deploy" {
  name = "gh-actions-deploy-email-${terraform.workspace}"
  ...
  tags = { Project = "news-aggregator", Module = "email" }
}
    ...
    Statement = [
      {
        ...
        Resource = "arn:aws:s3:::news-aggregator-lambda-artifacts-${data.aws_caller_identity.current.account_id}/email/*"
      },
      {
        Effect   = "Allow"
        Action   = ["s3:GetObject", "s3:PutObject", "s3:DeleteObject"]
        Resource = "arn:aws:s3:::news-aggregator-tf-state-${data.aws_caller_identity.current.account_id}/email/*"
      },
      {
        Effect   = "Allow"
        ...
        Condition = {
          StringLike = { "s3:prefix" = ["email/*", "email"] }
        }
      },
      {
        Effect = "Allow"
        ...
        Resource = "arn:aws:lambda:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:function:news-email-${terraform.workspace}"
      },
      {
        Effect = "Allow"
        Action = [
        ...
        Resource = [
          "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/news-email-${terraform.workspace}",
          "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/gh-actions-deploy-email-${terraform.workspace}",
        ]
      },
      {
        Effect = "Allow"
        ...
        Resource = "arn:aws:logs:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:log-group:/aws/lambda/news-email-${terraform.workspace}*"
      },
      {
        Effect = "Allow"
        Action = [
        ...
        Resource = "arn:aws:cloudwatch:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:alarm:news-email-${terraform.workspace}-*"
      },
      {
        Effect   = "Allow"
...
output "gh_actions_role_arn" {
  ...
  description = "OIDC role ARN — paste into GitHub Environment vars as AWS_DEPLOY_ROLE_ARN_EMAIL."
}
